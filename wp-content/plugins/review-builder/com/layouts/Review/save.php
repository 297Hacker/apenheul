...
<!-- WooCommerce review template -->
							<div class="sgrb-woo-template-wrapper"<?php echo (@$sgrbDataArray['template'] == 'woo_review') ? '' : ' style="display:none;"';?>>
								...
								<div class="sg-row">
									<div class="sg-col-4">
										<label for="wooReviewShowTypeProduct"><input id="wooReviewShowTypeProduct" type="radio" value="showByProduct" name="wooReviewShowType"<?php echo (@$sgrbDataArray['wooReviewShowType'] == 'showByProduct') ? ' checked' : '' ;?>><?php echo _e('Show review on selected products', 'sgrb');?></label>
									</div>
								</div>
								<div class="sg-row">
									<div class="sg-col-6">
						...
						<div class="sg-box-content">
							...
							<div class="tagchecklist">
								<?php if (@$sgrbDataArray['tags']) :?>
									...
									<?php foreach ($sgrbDataArray['tags'] as $tag) :?>
											<span id="sgrb-tag-index-<?=$index?>"><a href="javascript:void(0)" id="post_tag-check-num-<?=$index?>" onclick="SGRB.prototype.deleteTag(<?=$index?>)" class="ntdelbutton" tabindex="<?=$index?>"></a></span><span id="sgrb-tag-<?=$index?>" class="sgrb-new-tag-text"><?php echo $tag;?></span> <input type="hidden" value="<?php echo $tag;?>" name="tagsArray[]">
										<?php $index++;?>
									<?php endforeach;?>
								<?php endif ;?>
							</div>
							...
							<?php if (!@empty($allTags)) :?>
								<?php foreach ($allTags as $tags) :?>
									<a href="javascript:void(0)" class="tag-link-10 tag-link-position-1" title="2 topics" onclick="SGRB.prototype.setTags('<?php echo $tags->name;?>');" style="font-size: 22pt;"><?php echo $tags->name;?></a>
								<?php endforeach ;?>
							<?php endif;?>
							</p>
						</div>
