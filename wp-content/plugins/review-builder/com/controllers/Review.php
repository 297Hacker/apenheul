...
class SGRB_ReviewController extends SGRB_Controller
{
	...
	public function ajaxSave()
	{
		...
		if (count($_POST)) {
			...
			$successCommentText = @$_POST['success-comment-text'];
			$totalRatingText = @$_POST['total-rating-text'];
			$addReviewText = @$_POST['add-review-text'];
			$nameText = @$_POST['name-text'];
			$namePlaceholderText = @$_POST['name-placeholder-text'];
			$emailText = @$_POST['email-text'];
			$emailPlaceholderText = @$_POST['email-placeholder-text'];
			$titleText = @$_POST['title-text'];
			$titlePlaceholderText = @$_POST['title-placeholder-text'];
			$commentText = @$_POST['comment-text'];
			$commentPlaceholderText = @$_POST['comment-placeholder-text'];
			$postButtonText = @$_POST['post-button-text'];
			$captchaText = @$_POST['captcha-text'];
			$loggedInText = @$_POST['logged-in-text'];
			$noCategoryText = @$_POST['no-category-text'];
			$noNameText = @$_POST['no-name-text'];
			$noEmailText = @$_POST['no-email-text'];
			$noTitleText = @$_POST['no-title-text'];
			$noCommentText = @$_POST['no-comment-text'];
			$noCaptchaText = @$_POST['no-captcha-text'];
			//////////////////////
			$options['notify'] = '';
	...
	private function createReviewHtml($review, $isWidget=false)
	{
		...
		if ($commentsArray && @$review[0]['options']['show-comments'] == 1) {
			...
			$allApprovedComments .= '<a class="sgrb-comment-load" href="javascript:void(0)">Laad meer</a>';
			$allApprovedComments .= '</div>';
		}
...
// template options first column
		...
		if (!$userLoggedIn) {
			...
			$commentForm .= @$sgrvNotApprovedMessage.'<div class="sgrb-front-comment-rows">
