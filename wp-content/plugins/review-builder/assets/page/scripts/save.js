...
SGRB.prototype.init = function(){
	...
	if (jQuery('#wooReviewShowTypeProduct').attr('checked')) {
	...
	jQuery('#wooReviewShowTypeProduct').click(function(){
	...
	*/
	jQuery('.sgrb-tagadd').click(function(){
		SGRB.prototype.setTags(hasTags = false);
	});
	...
	if (jQuery('input[name=sgrb-template]').val() == 'woo_review') {
		if (jQuery('#wooReviewShowTypeProduct').attr('checked')) {
			...
			jQuery('.sgrb-woo-product').each(function(){
				if (jQuery(this).attr('checked') && !jQuery(this).attr('disabled')) {
		...
		else if (jQuery('#wooReviewShowType').attr('checked')) {
			var id = '';
...
SGRB.ajaxPagination = function(page,itemsRangeStart,perPage,reviewId){
	...
	ajaxHandler.callback = function(response){
		...
		for(var i in obj) {
			...
			commentHtml += '</div></div></div><div class="sg-row"><div class="sg-col-12"><span class="sgrb-name-title-text"><b></b><i>Gereageerd door </i><b> '+name+'</b></span></div></div>';
			commentHtml += '</div>';
		}
			sgrbMainWrapper.find('.sgrb-approved-comments-to-show').append(commentHtml);
			/*if (sgrbMainWrapper.find('.sgrb-load-it').length) {
				sgrbMainWrapper.find('.sgrb-common-wrapper').find('.sgrb-approved-comments-to-show').append(commentHtml);
			}
			else {
				sgrbMainWrapper.find('.sgrb-approved-comments-to-show').append(commentHtml);
			}*/
...
SGReview.prototype.checkRateClickedCount = function() {
	SGRB.prototype.ajaxUserRate(reviewId = false, isFirstClick = true);
}
