jQuery(document).ready(function(){
	...
	jQuery('.sgrb-reviewId').each(function(){
		...
		if (!mainWrapper.find('.sgrb-show-tooltip span').text().startsWith(0)) {
			mainWrapper.find('.sgrb-show-tooltip span').text().startsWith = function(searchString, position) {
				position = position || 0;
		...
		if (type == SGRB.sgrbRateTypeStar) {
				...
				if (mainWrapper.length > 0) {
					...
					if (mainWrapper.find('.sgrb-tooltip-text').text().startsWith('no rates')) {
						mainWrapper.find('.sgrb-rate-each-skin-wrapper').each(function(){
							jQuery(this).find('.rateYoTotal').rateYo({
								ratedFill: skinColor,
								readOnly: true
