...
SGRB.prototype.init = function(){
	...
	if (jQuery('#sgrbWooReviewShowTypeProduct').attr('checked')) {
...
SGRB.ajaxPagination = function(page,itemsRangeStart,perPage,reviewId){
	...
	ajaxHandler.callback = function(response){
		...
		for(var i in obj) {
			...
			commentHtml += '</div></div></div><div class="sg-row"><div class="sg-col-12"><span class="sgrb-name-title-text"><b>'+obj[i].date+'</b><i> , gereageerd door </i><b>'+name+'</b></span></div></div>';
			commentHtml += '</div>';
		}
